...
export class GetPageData {
    public async run(request: NextRequest) {
        ...
        if (pageRoute === "inicio") {
            return NextResponse.json(await this.loadInicioPage(Number(IdUser), UserName))
        }

        if (pageRoute === "categorias") {
            return NextResponse.json(await this.loadCategoriasPage(Number(IdUser), searchParams))
        }

        if (pageRoute === "personalizacao") {
            return NextResponse.json(await this.loadPersonalizacaoPage(Number(IdUser)))
        }

        return NextResponse.json({ pageRoute })
    }

    private loadInicioPage(IdUser: number, UserName: string): Promise<UtilTypes.InicioPageData> {
    ...
    private loadCategoriasPage(IdUser: number, searchParams: URLSearchParams): Promise<UtilTypes.CategoriasPageData> | void {
        let { month, year } = serverUtilsUseCases.getMonthYear()
    ...
    private loadPersonalizacaoPage(IdUser: number) {
