...
export function ConfirmDestinyDialog(props: Props) {

    let form = useForm<{ IdNew: string }>()

    let buttonClass = "w-fit rounded-lg p-2 px-4"

    let destinys = props.destinys.filter((item) => item.IdDestiny !== props.IdDelete)
        ...
        <Dialog open={true} PaperProps={{ className: "w-1/4 max-md:w-4/5 bg-default text-white" }}>
            ...
            <DialogContent>
                <div className="flex flex-col gap-5">
                    Esse destino possui {props.ExpensesQuantity} gastos atrelados a ele, o que vocês gostaria de fazer com eles?

                    <Select
                        form={form}
                        formProp="IdNew"
                        label="Novo destino" selectItems={destinys.map((item) => ({ key: item.IdDestiny.toString(), text: item.Name }))}
                    />
