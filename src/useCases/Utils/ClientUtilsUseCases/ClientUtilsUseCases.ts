...
import { GetExpenseType } from "./sections/getExpensePrice"
...
export class ClientUtilsUseCases {
    ...
    roundTo(value: number, round: number) {
        let remainder = value % round
        let roundHalf = round / 2

        if (remainder >= roundHalf) {
            return Math.floor(value / round) * round + round
        }
